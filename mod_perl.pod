...
=item Memory Balancing Servers

It is possible to reduce the number of server children bloated with a
Perl syntax tree inside of it by dedicating servers running on
different ports to specific tasks.  For instance, a server
configuration on one port (built without mod_perl) serves nothing but
image files, another server on a different port, serves nothing but
mod_perl scripts.  
