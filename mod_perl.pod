...
This script will be loaded when the server starts. See eg/MyPackage.pl
...
e.g. 
in the PerlScript MyPackage.pl this is "MyPackage::handler".
...
to step in and do something.  The Apache documentation will tell you all about
...
Two subroutines 'OK' and 'DECLINED' are defined when the server starts to
help you configure nested Perl*Handlers.  Example, let's say you want to 
configure a default PerlLogHandler and turn it off for certain 
sub-directories or files.

 <Location />
 PerlLogHandler MyLogger
 ....
 </Location> 

 #don't log requests for gif files
 <Files *.gif>
 PerlLogHandler DECLINED
 </Files>

=head1 I/O 

Apache's i/o is not stream oriented.  So, unless you have perl version
5.003_93 or higher, by default, you cannot 
...
In post 5.003 versions of Perl, two mechanisms have been introduced which allows
redirecting the STDIN and STDOUT streams.  
