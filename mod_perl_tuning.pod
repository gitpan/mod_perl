...
you have already built and installed a mod_perl enabled Apache
server.  Please also read the mod_perl documentation that comes with
mod_perl for programming tips.
...
programs directory, or the *.perl files).  Here is an example:
 ...
 # files.  Changes made to these functions will require killing and restarting
 # the httpd since it does not re-initialize Perl (as of mod_perl 1.00).
...
What this does is pull in all of the code used by any program into the
main HTTPD process, which then creates the child processes with the
code already in place.  You can also put any new modules you like into
the F</var/www/perllib> directory and simpley C<use> them in your
...
any C<use Foo;> statement in any commonly used program.  The idea is
that the operating system's VM subsystem will share the data across
the processes.

The only problem with this is that you can't pre-load the scripts
called by Apache::Registry.  So how do you work around that?  The
thing to do is to put all of your code into a module, say
F<VicksProgram.pm> in the F</var/www/perllib> directory.  Then, in
your F<startup.perl> file, add a C<use VicksProgram;> line.  Now, your
registry program becomes something like this:

 use strict;
 use VicksProgram;
 VicksProgram::main();

Where the function main() in the VicksProgram module is the thing that
does the real work.  The code is now shared among all HTTPD processes,
which I<should> reduce total memory usage.
...
content will be served up by lots of small HTTPD processes (I<not>
configured to use mod_perl), and the relatively few mod_perl requests
...
C<http://squid.nlanr.net/Squid/> between the clients and your large
...
that do not change on every view.
...
directive allows these multiple servers to be connected transparently
to the users.
...
If you have contributions for this document, please send them to me.
Perl POD format is best, but plain text will do, too.  If you need
assistance, contact the mod_perl mailing list at
